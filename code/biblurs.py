"""
biblurs.py - performs a series of bilateral blurs for creating normals
...
def blurs(image_inst):
    ...
    print("blur0")
    image_inst.cv_blur1 = cv2.bilateralFilter(image_inst.cv_blur0, 3, 3, 3, image_inst.cv_blur1, cv2.BORDER_WRAP)
    cv2.imwrite('..\\images\\blur1.png', image_inst.cv_blur1)
    print("blur1")
    image_inst.cv_blur2 = cv2.bilateralFilter(image_inst.cv_blur1, 9, 9, 9, image_inst.cv_blur2, cv2.BORDER_WRAP)
    cv2.imwrite('..\\images\\blur2.png', image_inst.cv_blur2)
    print("blur2")
    image_inst.cv_blur3 = cv2.bilateralFilter(image_inst.cv_blur2, 27, 27, 27, image_inst.cv_blur3, cv2.BORDER_WRAP)
    cv2.imwrite('..\\images\\blur3.png', image_inst.cv_blur3)
    print("blur3")
    image_inst.cv_blur4 = cv2.bilateralFilter(image_inst.cv_blur3, 81, 81, 81, image_inst.cv_blur4, cv2.BORDER_WRAP)
    cv2.imwrite('..\\images\\blur4.png', image_inst.cv_blur4)
    print("blur4")
    image_inst.cv_blur5 = cv2.bilateralFilter(image_inst.cv_blur4, 163, 163, 163, image_inst.cv_blur5, cv2.BORDER_WRAP)
    cv2.imwrite('..\\images\\blur5.png', image_inst.cv_blur5)
    print("blur5")
    image_inst.cv_blur6 = cv2.bilateralFilter(image_inst.cv_blur5, 251, 251, 251, image_inst.cv_blur6, cv2.BORDER_WRAP)
    ...
    cv2.imwrite('..\\images\\b0_vhf.png', vhf.astype(np.uint8))
    hf = (image_inst.hf + 1)/2 * 255
    cv2.imwrite('..\\images\\b1_hf.png', hf.astype(np.uint8))
    mhf = (image_inst.mhf + 1)/2 * 255
    cv2.imwrite('..\\images\\b2_mhf.png', mhf.astype(np.uint8))
    mf = (image_inst.mf + 1)/2 * 255
    cv2.imwrite('..\\images\\b3_mf.png', mf.astype(np.uint8))
    mlf = (image_inst.mlf + 1)/2 * 255
    cv2.imwrite('..\\images\\b4_mlf.png', mlf.astype(np.uint8))
    lf = (image_inst.lf + 1)/2 * 255
    cv2.imwrite('..\\images\\b5_lf.png', lf.astype(np.uint8))
    vlf =(image_inst.vlf + 1)/2 * 255
    cv2.imwrite('..\\images\\b6_vlf.png', vlf.astype(np.uint8))
    # cv2.imwrite('..\\images\\b6_vlf.png', image_inst.vlf.astype(np.int8))

    cv2.imshow('ivhf', vhf.astype(np.uint8))
    cv2.imshow('ihf', hf.astype(np.uint8))
    cv2.imshow('imhf', mhf.astype(np.uint8))
    cv2.imshow('imf', mf.astype(np.uint8))
    cv2.imshow('imlf', mlf.astype(np.uint8))
    cv2.imshow('ilf', lf.astype(np.uint8))
    cv2.imshow('ivlf', image_inst.vlf.astype(np.uint8))
    cv2.waitKey(); cv2.destroyAllWindows()

    all_img = (image_inst.vhf + image_inst.hf + image_inst.mhf + image_inst.mf + image_inst.mlf +
               image_inst.lf + image_inst.vlf)
    all_img_int = (((all_img + 1)/2) * 255)
    ...
    cv2.imwrite('..\\images\\b_all_freq.png', all_freq.astype(np.uint8))
...
def initialize_blurs(image_inst):
    ...
    blank_gray = np.zeros(shape, dtype=np.uint8)
